...
Tile Board::getTile(int _player_position){
    Tile tile;
    tile = _tiles[_player_position];
    return tile;
}

void Board::tileNames(){
    for(int i = 0; i < 83; i++){
        if((i % 3) == 0){
            _tiles[i].color_name = "Purple";
        }else if ((i % 3)== 1){
           _tiles[i].color_name  = "Green";
            
        }else if((i%3)== 2){
            _tiles[i].color_name = "Blue";
        }
